...
class ComponentRegistry:
    ...
    def _load_components(self) -> None:
        ...
        for _, name, _ in pkgutil.iter_modules(["pysyslog.inputs"]):
            module = importlib.import_module(f"pysyslog.inputs.{name}")
            if hasattr(module, "Input"):
                self.input_types[name] = module.Input
        
        # Load parser components
        for _, name, _ in pkgutil.iter_modules(["pysyslog.parsers"]):
            module = importlib.import_module(f"pysyslog.parsers.{name}")
            if hasattr(module, "Parser"):
                self.parser_types[name] = module.Parser
        
        # Load output components
        for _, name, _ in pkgutil.iter_modules(["pysyslog.outputs"]):
            module = importlib.import_module(f"pysyslog.outputs.{name}")
